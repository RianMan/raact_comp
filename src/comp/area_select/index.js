...
export default class AddReceive extends React.Component {
  ...
  componentDidMount(){
    ...
    let provinceChildren = document.querySelectorAll('#province .item');
    let city = document.querySelector('#city');
    let county = document.querySelector('#county');
    let provinceTop = Array.from(provinceChildren).map(v => v.offsetTop);
    console.log(provinceTop,'provinceTop');
    province.addEventListener('scroll',(e)=>{
    ...
    switch (name) {
        case "province":
            this.setState({
                ...
                cities: this.getCity(selectedValue),
                city:this.getCity(selectedValue)[0],
                county: this.getCounty(selectedValue, this.getCity(selectedValue)[0])[0],
                counties:this.getCounty(selectedValue, this.getCity(selectedValue)[0])
            });
            break;
        case "city":
            this.setState({
                city: selectedValue,
                counties: this.getCounty(this.state.province, selectedValue),
                county:this.getCounty(this.state.province, selectedValue)[0]
            });
            break;
        case "county":
            this.setState({
  ...
  handleOk = () => {
    this.props.handleOk()
    this.props.handleControllerShow(false)
  }

  render() {
    const { province:p , city:ci, county:co} = this.state;
    const { show,handleControllerShow } = this.props;
